...
| 言語 | C# 12 |
| FW | .NET 10 / .NET MAUI |
| TFM | `net10.0-android`（App/Core/Data）。Relay は `net10.0` |
| 最小OS | Android 13 / API 33 |
| IDE | Visual Studio 2026（MAUIワークロード。.NET 10 は VS 2022 非対応） |
...
| System.Text.Json | JSON | 標準（.NET 10 同梱。別途参照不要） |
| sqlite-net-pcl | SQLite ORM | `1.*`（+ `SQLitePCLRaw.bundle_green 2.*`。TBird.Maui.DB が参照済みで推移取得。直接参照不要） |
| Xamarin.AndroidX.Work.Runtime | WorkManager | `2.11.2`（net10.0-android36 でもビルド確認済。App が直接参照） |
| Plugin.LocalNotification | ローカル通知 | `13.0.0`（**13.0.0 は net10.0-android36 もサポート**するため net10 移行後も据え置き。14.x へ上げる場合はタップ EventArgs 名前空間等の API 変更に注意。App が直接参照） |
| CommunityToolkit.Maui | トースト/スナックバー | `13.*`（**.NET 10 対応は 13.x**。12.x は .NET 9 専用。App が直接参照。`builder.UseMauiCommunityToolkit()` が必要） |
...
- **トースト/スナックバー（手動チェック失敗時など）**: **既存ラッパー無し**。`CommunityToolkit.Maui`（`Toast` / `Snackbar`）を App プロジェクトに追加して使う。`builder.UseMauiCommunityToolkit()` を `MauiProgram` で呼ぶ。将来差し替え可能にするため、`Core/Abstractions` に `IUserNotifier`（例: `ShowToastAsync(string)` / `ShowSnackbarAsync(string)`）を定義し、App 層で CommunityToolkit 実装を DI 登録すること（要件「汎用部品はインターフェース越し」方針）。
  - ※ NuGet 表（§2）に記載済み（`CommunityToolkit.Maui` 13.* = .NET 10 対応）。
...
- **TBird.Maui.DB**（`net10.0`）— SQLite (sqlite-net-pcl) 基底＋マイグレーション枠組み。
...
- **TBird.Maui.Background**（`net10.0-android`）— 優先度ジョブキュー / ネットワーク監視 / サイト別レートリミッタ。**全て Singleton 登録必須**（`AddTransient`/`AddScoped` 禁止＝ハンドラリーク）。
...
- **TBird.Maui.Web**（`net10.0`）— HTTP transient エラー処理＋AngleSharp 解析。
...
- 参照の向き: `Data` → TBird.Maui.DB / TBird.Maui.Web、`App`/`Core` → TBird.Core / TBird.Maui、レート制限・ネット監視が要る層 → TBird.Maui.Background。`net10.0`（Web/DB）は `net10.0-android` から問題なく参照可能。
...
### 7.1 チェック処理は共通サービス化
「タイトル検索（ジャンル絞り込み＋NGKeyword除外付き） → 除外フィルタ → 著者集合の部分集合一致判定 → 差分判定 → INSERT → お気に入り自動登録 → 通知」を**単一の共通サービス**として実装し、自動（WorkManager）・手動（更新ボタン）の両方から呼ぶ。手動チェックもバックグラウンド実行で、検知時は通知を出す。

### 7.2 シリーズ同定（著者集合の部分集合一致＋ジャンル絞り込み）
- チェック時は**シリーズキー（タイトル語）＋メディア種別のジャンル（koboGenreId）をキーワードに検索**して候補巻を取得し、`正規化後の 登録時著者集合 ⊆ 検索結果側の著者集合`（**部分集合一致**＝登録時に選択した著者が候補側に全員含まれるか。候補側に未選択の著者が増えても対象）で同定する。著者名は検索キーにしない（同一著者の別シリーズ誤統合を防ぐ）。空集合ガード: 登録著者集合が空なら同定不成立（空集合は全候補の部分集合になるため）。
- **ジャンル絞り込み**: `MediaType.ToKoboGenreId(s.MediaType)` を検索に渡す（**ラノベ=`101903` / コミック=`101904`**。2026-06-08 ジャンル検索APIで実値検証済み。値の正は `Core/MediaType.cs`、`App/Models/KoboGenres.cs` はそれを参照）。**コミカライズ弁別は著者集合ではなくジャンルが担う**（旧仕様の集合一致は巻ごとの作画者差異で本編の正当な巻まで落とす問題があった）。
- **除外キーワード（NGKeyword）**: `_prefs.ExcludeKeywords` を半角スペース連結して `SearchByKeywordAsync(keyword, koboGenreId, ngKeyword, ct)` に渡し、分冊版等を**API段階で除外**（120件枠の節約。OR動作）。タイトル限定の `ContainsExcludeKeyword` は安全網として併用。中継サーバーは本文全キー透過のため改修不要。
- 著者文字列は区切り文字（`/` `,` 空白等）で分割して集合化。比較前に**正規化**（肩書きラベル「原作/作画/イラスト」等の除去・空白除去・全角/半角統一）。**人物名は除去せず保持**（イラストレーター/作画者をあえて登録著者に含めれば部分集合一致の条件を厳しくでき、より細かく弁別可能）。
- 巻数は**抽出も保持もしない**。種別フラグ（本編/外伝/短編集）も**持たない**。
- 副題の有無でシリーズを分けない（タイトル差異ではなく著者集合＋ジャンルで同定）。
...
2. パラメータ名（`koboGenreId`, `salesType`, `sort`, `NGKeyword` 等）と値。（`NGKeyword`=除外キーワード・スペース区切り複数可・OR動作は 2026-06-08 実機確認済）
3. レスポンスの発売日フィールド名・形式（和暦/未定文字列の有無）→ パース・正規化を調整。
4. 著者フィールドの区切り文字・肩書き混入の有無 → 分割・正規化を調整。
5. ランキング取得用 sort 値、ジャンルID体系。（**ジャンルID確定済 2026-06-08: ラノベ=`101903` / コミック=`101904`**。旧暫定値 LightNovel=101904/Comic=101901 は誤りだった）
