...
[`uv`](https://docs.astral.sh/uv/) 管理（`pyproject.toml`）。`.venv/` は `_Apps/ml/` 内（gitignore 済・`uv sync` で再生成可）。
学習済みモデル実体は **`_Tools/TradeAnalyzer/ml/models/`**（`train.py` が `__file__` からリポジトリルートを求めて解決。
`_Tools/` は gitignore 済＝`_Apps` 削除やブランチ切替でも生存する。C# の `AppPaths.MlModelsDir` と一致）。
...
`trade.db` は C# 側 `AppPaths` が **`_Tools/TradeAnalyzer/trade.db`** に絶対解決する（CWD 非依存＝`dotnet run`・
publish 済み exe・タスクスケジューラのいずれでも同一ファイル）。Python から実行するときは `_Apps/ml` を cwd に
...
| `_Tools/TradeAnalyzer/ml/models/` | 学習済みモデル（`*.txt` LightGBM native）＋`best_params.json`（gitignore 済） |
