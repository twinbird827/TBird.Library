...
namespace Netkeiba
{
    public partial class MainViewModel
    {
        ...
        public IRelayCommand S2EXEC => RelayCommand.Create(async _ =>
        {
            using (var conn = CreateSQLiteControl())
            {
                ...
                foreach (var raceid in rac)
                {
                    ...
                    var head = new[] { "ﾚｰｽID", "開催日数", "枠番", "馬番", "着順", "ﾗﾝｸ2", "馬ID" };
                    ...
                    if (create)
                    {
                        ...
                        await conn.ExecuteNonQueryAsync(Arr(
                            ...
                            head.Select(x => $"{x} INTEGER").GetString(","),
                    ...
                    {
                        keys = keys ?? ins.Keys.Where(x => !head.Contains(x)).ToArray();

                        var prms1 = head.Select(x => SQLiteUtil.CreateParameter(System.Data.DbType.Int64, ins[x]));
                        ...
                        await conn.ExecuteNonQueryAsync(
                            $"INSERT INTO t_model ({head.GetString(",")},Features) VALUES ({Enumerable.Repeat("?", head.Length).GetString(",")}, ?)",
        ...
        private float 他馬比較(Dictionary<string, object> dic, Dictionary<string, object>[] racarr, string key, float def, Func<IEnumerable<float>, float> func)
        {
            var x1 = dic[key].GetSingle();
            var x2 = GetSingle(racarr.Select(x => x[key].GetSingle()), float.NaN, func);
            return GetSingle(x1, x2, float.NaN, (x, y) => x - y);
        }
        ...
        private async Task<Dictionary<string, object>> GetAnalysis(SQLiteControl conn, int num, Dictionary<string, object> src, string keyname, string[] headnames)
        {
            ...
            var select = Arr(
                ...
                $"IFNULL(LOWER_QUARTILE(着順), {DEF["着順"] * 0.75})",
                $"IFNULL(UPPER_QUARTILE(着順), {DEF["着順"] * 1.25})"
            );
