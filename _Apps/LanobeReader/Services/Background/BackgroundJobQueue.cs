...
/// このラッパーが残る目的:
///   (a) コンストラクタで Repository / INovelServiceFactory 等の依存をキャプチャした processor
///       delegate を組み立てる場所として機能する
...
public class BackgroundJobQueue
{
        ...
        INovelServiceFactory serviceFactory)
    {
        _queue = new PriorityJobQueue<PrefetchEpisodeJob, int>(
            ...
            {
                // 既にキャッシュ済みならスキップ
                var cached = await cacheRepo.GetByEpisodeIdAsync(job.EpisodeDbId).ConfigureAwait(false);
