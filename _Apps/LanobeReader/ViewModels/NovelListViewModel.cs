...
public partial class NovelListViewModel : AutoReloadViewModel
{
    ...
    private async Task RefreshAsync()
    {
        ...
        {
            // IsLoading の解除はメインスレッドキューの後尾へ積む。CheckAllAsync が自身の新着を
            // UpdatesDetectedMessage で送ると、その配送(BeginInvokeOnMainThread)は本継続より前に
            // キュー済みのため、エコーのハンドラは IsLoading=true を見て抑止される。以降に到着する
            // (別経路の)新着は IsLoading=false 後に処理され確実に再読込される。これにより専用の
            // 自エコー抑止フラグを持たず、抑止条件を IsLoading 一本に統一できる。
