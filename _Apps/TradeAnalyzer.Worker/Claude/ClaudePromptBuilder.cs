...
internal static class ClaudePromptBuilder
{
    public static string Build(ClaudeFacts facts)
    {
        ...
        // 数値ガード（QualitativeNumberGuard）は表記完全一致で照合するため、単位換算された正当な引用
        // （例: 136,821,000,000円→「136,821百万円」）が注入外数値として誤検知される。換算自体を禁止して FP を断つ。
        sb.AppendLine("- 数値を引用するときは入力の表記のまま用いる。単位換算・丸め・言い換えをしない（例: 136,821,000,000円 を 136,821百万円 と書き換えない）。");
