...
v_group2 AS (SELECT 馬場, 距離, SUM(上り * CNT) / SUM(CNT) 上り, SUM(ﾀｲﾑ変換 * CNT) / SUM(CNT) ﾀｲﾑ変換 FROM v_group1 GROUP BY 馬場, 距離)
...
";
		...
		public static string GetRaceDetailSql()
		{
			return @"
SELECT h.ﾚｰｽID, h.ﾚｰｽ名, h.開催場所, h.距離, h.馬場, h.馬場状態, h.ﾗﾝｸ1, h.優勝賞金, h.開催日, h.頭数,
       d.枠番, d.馬番, d.馬ID, d.騎手ID, d.調教師ID, u.父ID, u.母父ID, u.生産者ID, d.着順, d.ﾀｲﾑ変換, (h.優勝賞金 / MIN(d.着順, h.頭数)) 賞金, u.評価額, u.生年月日, d.斤量, d.通過, d.上り, d.馬性, d.ﾀｲﾑ指数, d.着差, CAST(d.体重 AS REAL) 体重, CAST(d.増減 AS REAL) 増減, CAST(d.単勝 AS REAL) 単勝,
...
FROM   t_orig_h h
CROSS JOIN t_orig_d d ON h.ﾚｰｽID = d.ﾚｰｽID
CROSS JOIN t_uma u ON d.馬ID = u.馬ID
CROSS JOIN t_oikiri o ON d.ﾚｰｽID = o.ﾚｰｽID AND d.馬ID = o.馬ID
WHERE  1 = 1
";
		}
...
";
			...
			{
				var raceid = reader.GetValue(0).Str();
				...
				if (currentRace == null || currentRace.RaceId != raceid)
				{
					currentRace = new Race(reader);
				}
				buffer.Add(new RaceDetail(reader, currentRace));
			}
