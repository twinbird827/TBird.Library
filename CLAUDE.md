...
`TradeAnalyzer.*`（`TradeAnalyzer.Data` / `.Core` / `.Worker`）は **TBird.* を参照しない独立した net10.0 多プロジェクトアプリ**であり、以下の全体共通ルールは適用外とする:

- **`TBirdObject` 継承 / `Dispose` オーバーライド規則**: TBird.Core 非参照のため適用しない（リソース解放は標準 `IDisposable` で行う）。
- **`{型名}Extension.cs` 命名規則 / 名前空間 `TBird.{レイヤー}.{機能}`**: 名前空間は `TradeAnalyzer.*`。
- **プロジェクトフォルダを作らない規則**: EFCore データ層 / ドメイン / Worker の関心分離と TFM 独立性のため、`TradeAnalyzer.Data` 等の複数プロジェクト（フォルダ）構成を採る。

理由: 規約を強制すると TBird.Core への参照追加・TFM 緊張・Dispose 再設計が必要になり、独立アプリの利点（依存最小・TFM 独立）を損なうため。
