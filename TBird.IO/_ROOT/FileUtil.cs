...
namespace TBird.IO
{
    public static class FileUtil
    {
        ...
        {
            var work = Directories.RootDirectory.TrimEnd('\\');
        ...
        public static IEnumerable<string[]> CsvLoad(string file)
        {
            ...
            using (var fs = new FileStream(file, FileMode.Open, FileAccess.Read, FileShare.Read, _buffersize, true))
        ...
        public static void DirectoryCopy(string src, string dst)
        {
            ...
            // ﾃﾞｨﾚｸﾄﾘのｺﾋﾟｰ（再帰を使用）
            foreach (DirectoryInfo directoryInfo in srcdi.GetDirectories())
            {
        ...
        /// <returns></returns>
        public static Task FileCopyAsync(string src, string dst, bool overwrite = true)
        {
            var cts = new CancellationTokenSource();
            // ﾀﾞﾐｰのｷｬﾝｾﾙﾄｰｸﾝを指定してｺﾋﾟｰ
            return FileCopyAsync(src, dst, overwrite, cts);
        }
        ...
        /// <param name="src">ｺﾋﾟｰ元ﾌｧｲﾙ</param>
        /// <param name="dst">ｺﾋﾟｰ先ﾌｧｲﾙ</param>
        /// <param name="cts">ｷｬﾝｾﾙﾄｰｸﾝ</param>
        /// <returns></returns>
        public static async Task FileCopyAsync(string src, string dst, bool overwrite, CancellationTokenSource cts)
        {
            if (overwrite && File.Exists(dst))
            {
                File.Delete(dst);
            }
            using (var ss = new FileStream(src, FileMode.Open, FileAccess.Read, FileShare.Read, _buffersize, true))
            using (var ds = new FileStream(dst, FileMode.Create, FileAccess.Write, FileShare.None, _buffersize, true))
            {
                await ss.CopyToAsync(ds, _buffersize, cts.Token);
            }
        }
